...
export default function Loans() {
  ...
  async function openNew() {
    if (isClient) {
      setForm({ type: 'personal', currency: 'DOP', frequency: 'monthly', term_months: 3, rate_monthly: 10, cuotas_manual: '', client_id: myClientId || '' })
  ...
  async function save() {
    ...
    try {
      ...
      const reqs = isClient
        ? [['amount_requested','Monto'],['term_months','Plazo'],['purpose','Propósito'],['monthly_income','Ingreso']]
        : [['client_id','Cliente'],['amount_requested','Monto'],['term_months','Plazo'],['purpose','Propósito'],['monthly_income','Ingreso']]
      for (const [c, l] of reqs) { if (!form[c]) throw new Error(`El campo "${l}" es obligatorio`) }
      if (isClient && !clientIdToUse) throw new Error('No se pudo identificar tu registro de cliente. Contacta a soporte.')
      if (selected?.id) {
        if (isClient) throw new Error('No tienes permiso para editar solicitudes.')
        ...
        await db.createLoanApplication({
          client_id: clientIdToUse, product_id: form.product_id || null, type: form.type || 'personal',
